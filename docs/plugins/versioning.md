...
1.5 row above pins `min_app_version="2.30.0-beta.1"` and not `"2.30.0"`.

That hyphenated value is deliberate, and getting it wrong fails in the
direction that hurts most. `min_app_version` is compared with
[PEP 440](https://peps.python.org/pep-0440/) ordering, which treats a
prerelease as **earlier** than the release it leads to — `2.30.0-beta.1`
normalises to `2.30.0b1`, and `2.30.0b1 < 2.30.0`. So:

| your pin | on 2.29.0 | on 2.30.0-beta.1 | on 2.30.0-beta.2 | on 2.30.0 | on 2.31.0 |
| --- | --- | --- | --- | --- | --- |
| `"2.30.0"` (the stable) | refused | **refused** | **refused** | loads | loads |
| `"2.30.0-beta.1"` | refused | loads | loads | loads | loads |

**Pinning the stable release refuses every beta host — the exact users the
beta exists for.** Your add-on would be listed as incompatible for everyone
running the build that first has the feature, and would start working only
once the stable release landed. Pinning the prerelease admits the beta, every
later beta, the release candidate and every stable release after it, while
still refusing the release before — which is the whole job of the pin.

Two practical notes:

- **Write the SemVer spelling with the hyphen** (`2.30.0-beta.1`). PEP 440's
  `2.30.0b1` means the same thing to the comparison, but the hyphenated form
  is what the release page and the app's own About box show, so it is what
  your users can check against.
- **You do not need to chase the beta counter.** Pinning `beta.1` is
  satisfied by `beta.2`, `beta.3` and `rc1`, so you only revisit the pin if
  the *minor* changes.
