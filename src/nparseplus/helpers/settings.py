...
from nparseplus.helpers import config, text_time_to_seconds
from nparseplus.parsers.spells import CustomTrigger
...
class SettingsWindow(QDialog):
        ...
        for stacked_widget in self._widget_stack.findChildren(QFrame):
            for widget in stacked_widget.children():
                ...
                if wt == QCheckBox:
                    ...
                    if key1 == "sharing" and key2 == "discord_channel" and config.data[key1][key2]:
                        self.sharing_group_key.setDisabled(True)
                    if (
                        key1 == "sharing"
                        and key2 == "player_name_override"
                    ...
                    widget.setStyleSheet(
                        f"background-color: {hexcolor}; border: 4px solid {hexcolor};"
                    )
        for widget in self._color_dialogs.values():
            wt = type(widget)
            if wt == QColorDialog:
    ...
    def _create_settings(self):
        ...
        shsl.addRow("Display Name", self.sharing_player_name)

        sharing_player_name_override = QCheckBox()
        sharing_player_name_override.setObjectName("sharing:player_name_override")
        shsl.addRow("Display Name Override", sharing_player_name_override)
        sharing_player_name_override.clicked.connect(
            functools.partial(
                self._dynamic_field_toggle,
        ...
        shsl.addRow("Sharing Hostname", sharing_hostname)

        self.sharing_group_key = QLineEdit()
        self.sharing_group_key.setObjectName("sharing:group_key")
        shsl.addRow("Group Key", self.sharing_group_key)
        ...
        sharing_discord_channel.clicked.connect(
            functools.partial(
                self._dynamic_field_toggle, sharing_discord_channel, self.sharing_group_key, False
            )
        )
        ...
        for window in ("maps", "spells", "discord"):
            ...
            if window == "discord":
                ...
                color_button.clicked.connect(
                    functools.partial(self.show_color_picker, window=window, preview=color_preview)
                )
                color_preview.clicked.connect(
                    functools.partial(self.show_color_picker, window=window, preview=color_preview)
                )
    ...
    def show_color_picker(self, window, preview):
        ...
        if dialog.exec():
            ...
            preview.setStyleSheet(f"background-color: {hexcolor}; border: 4px solid {hexcolor};")
        else:
...
class CustomTriggerSettings(QDialog):
    ...
    def _save_to_config(self):
        config.data["spells"]["custom_timers"] = [
            x.to_list()
            for x in sorted(self._custom_triggers.values(), key=lambda x: x.name)
            if x.name != ""
        ]
        config.save()
    ...
    def _save_trigger(self):
        if self._trigger_name.text():
            ...
            if self._trigger_text.text() and text_time_to_seconds(self._trigger_time.text()):
                if (
                    self._trigger_name.text() in self._custom_triggers
                    and not self._trigger_name.text() == self._current_trigger
                ):
                    m = QMessageBox()
