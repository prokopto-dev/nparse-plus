...
class Discord(ParserWindow):
    ...
    def _fix_background(self):
        if self.overlay:
            ...
            bg_color_set = JS_ADD_CSS_TEMPLATE % {
                "name": "bgcolor",
                "new_css": f"body {{ background-color: rgba({qcolor.red()},{qcolor.green()},{qcolor.blue()},{self._bg_opacity / 100}) !important }}",
            }
    ...
    def show_settings(self):
        ...
        settings_webview.load(QtCore.QUrl("https://streamkit.discord.com/overlay"))
        settings_layout.addWidget(settings_webview, 0, 0, 1, 2)

        ok_button = QPushButton("Save")
        ok_button.clicked.connect(
            functools.partial(self._save_settings, settings_webview, self.settings_dialog)
        )
    ...
    def _save_settings(self, webview, frame):
        get_url = (
            "document.querySelector('[class^=\"Config_configLink\"]')"
    ...
    def _skip_stream_button(self, webview):
        ...
        chooseVoice = "document.querySelectorAll('button[value=voice]')[0].click();"
        toggleSmallAvatars = (
        ...
        hide_header = JS_ADD_CSS_TEMPLATE % {"name": "hideheader", "new_css": CSS_HIDE_HEADER}
