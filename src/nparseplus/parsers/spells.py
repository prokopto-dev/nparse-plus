...
from nparseplus.helpers import config, format_time, text_time_to_seconds
...
class Spells(ParserWindow):
    ...
    def __init__(self):
        self.name = "spells"
        super().__init__()
        QApplication.instance()._signals["settings"].spell_triggers_updated.connect(
    ...
    def parse(self, timestamp, text):
        ...
        # Spell Interrupted
            ...
            (self._spell_triggers and text[:26] == "Your spell is interrupted.")
    ...
    def load_custom_timers(self):
        ...
        for item in config.data["spells"]["custom_timers"]:
            ...
            rx = re.compile("^{}$".format(ct.text.replace("*", ".*")), re.RegexFlag.IGNORECASE)
            self._custom_timers[rx] = ct

    def _toggle_custom_timers(self, _):
        config.data["spells"]["use_custom_triggers"] = self._custom_timer_toggle.isChecked()
        config.save()
...
class SpellTarget(QFrame):
    ...
    def add_spell(self, spell, timestamp):
        ...
        for x, widget in enumerate(
            sorted(self.findChildren(SpellWidget), key=lambda x: x.end_time - now)
        ):
        ...
        if config.data["spells"]["use_casting_window"]:
            ...
            self._times_up_timer.start(
                int(
                    ...
                    + config.data["spells"]["casting_window_buffer"]
                    - msec_offset
                )
            )
...
def get_spell_duration(spell, level):
    if spell.name in config.data["spells"]["use_secondary"] or (
        config.data["spells"]["use_secondary_all"] and spell.type == 0
    ):
    ...
    if formula == 0:
        spell_ticks = 0
    if formula == 1:
        spell_ticks = int(math.ceil(level / 2.0))
        spell_ticks = min(spell_ticks, duration)
    if formula == 2:
        spell_ticks = int(math.ceil(level / 5.0 * 3))
        spell_ticks = min(spell_ticks, duration)
    if formula == 3:
